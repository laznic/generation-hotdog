...
export default function Room () {
  ...
  useEffect(function checkRoomExistsOnMount() {
    async function fetchRoomData () {
      const { error, data: room } = await supabase.from('hotdogs')
        .select(`status, image`)
        .eq('code', roomId)
      ...
      className={'mt-[7vw] relative container mx-auto grid grid-cols-2 gap-8'}>
        ...
        <section>
          <h1 className="relative">
            ...
            <p className="font-kanji text-4xl md:text-5xl 2xl:text-7xl tracking-tighter text-neutral-100">
              絵文字を選ぶ
            </p>
          </h1>
          ...
          <section className="mb-12">
            <h2 className="mb-2 text-6xl">
              ...
              <p className="font-kanji text-3xl md:text-2xl 2xl:text-4xl tracking-tighter text-neutral-100">
                シェアルーム
              </p>
            </h2>
        ...
        <section className="grid mt-10">
          <section className="flex items-start gap-6 justify-end flex-wrap">
            {players?.map((player) => (
              <article key={player.creatorId} className={`relative grid items-center justify-center text-center w-fit ${player.ready || (isSelf(player.creatorId) && readyInState) ? 'text-neutral-100' : 'text-neutral-700'}`}>
                {player.ready || (isSelf(player.creatorId) && readyInState) ? (
                  <article className="absolute text-neutral-100 rounded-full top-4 -left-5">
